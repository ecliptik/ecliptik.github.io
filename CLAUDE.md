...
**Generate Gopher:** `python3 _scripts/manage.py gopher` (RFC1436-compliant, 30 markdown posts)
...
**Generate gopher content from Jekyll blog:**
```bash
...
**Testing:**
```bash
bash tests/verify_gopher.sh                    # Verify structure
docker compose -f docker-compose.gopher.yml up -d  # Start test server
curl gopher://localhost:70/                    # Test with curl
lynx gopher://localhost:70                     # Test with lynx
```
