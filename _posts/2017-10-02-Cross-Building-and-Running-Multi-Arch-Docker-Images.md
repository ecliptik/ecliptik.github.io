...
Building an image is similiar to Docker For Mac, take an existing `Dockerfile` and replace it's `FROM` with a corresponding [Multiarch Docker Hub image](https://hub.docker.com/u/multiarch/).
